...
#include "ebOBJECTIO_s.h"    //base class
//#include <windows.h>
#include <math.h>          //sqrt
...
class VESS_s: public OBJECTIO_s
   {
   public:
    VESS_s();
   VESS_s(const VESS_s&);
   ~VESS_s();
   VESS_s& operator = (VESS_s& a);
   void Releasemem();
    ...
    VESS_s& ConcatenateVessels(VESS_s& a, VESS_s& b);
   ...
   protected:
      ...
      int LoadHeader(std::istream* ifile)   {return OBJECTIO_s::LoadHeader(ifile);}
      int LoadPoints(std::istream* ifile);
