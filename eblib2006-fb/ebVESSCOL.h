...
class VESSCOL:public FILEHEADERIO
   {
   public:
      VESSCOL();
      ~VESSCOL();
        VESSCOL(const VESSCOL& a);
        VESSCOL& operator = (const VESSCOL&a);
      ...
      VESS* GetVess()            {return vess;}   
      VESS* GetVess(int i)          {return &vess[i];}
      ...
      int LoadWorldTree(int n, VESS* v, int* vox, float* nsteps);
      int LoadVoxTree(int n, VESS* v, int* vox, float* nsteps);

        
                        //This group is used when creating reconstructed vessels that
                        //must be added to a vessel collection that may or may not be 
                        //empty. You are restricted to adding by the VESSCOLMAXINCREMENT
        ...
        int AddWorldVess(VESS* avess);
        int AddVoxVess(VESS* avess);
        int RemoveLastVess();
      ...
      void RenumberParent(VESS* v, LIST<int>* discards);
      void RenumberChildren(VESS* v, LIST<int>* discards);
