...
const int VESSCOL_sMAXINCREMENT = 20;      //extra vessels for splitchild ops
...
VESSCOL_s::VESSCOL_s()
   ...
   maxvess = VESSCOL_sMAXINCREMENT;    //can add a limited number of vessels

   try
      {
      vess = new VESS_s[maxvess];         //set default to this limited number.
                                       //File load can set any arbitrary number.
      if (!vess)
         {
         maxvess = 0;
         std::cerr<<"No mem for vessel creation\n";
         strcpy_s(err,200, "No mem for vessel creation");
         }
      }
   catch(...)
      {
...
//----------------------------------------
VESSCOL_s::VESSCOL_s(const VESSCOL_s& a)
   {
   int i;
   ...
   if (vessnum >= maxvess)
      maxvess = vessnum + VESSCOL_sMAXINCREMENT;

   try
      {
      vess = new VESS_s[maxvess];
      if (!vess)
         {
         vessnum = maxvess = 0;
         std::cerr<<"No mem for vessel creation\n";
...
//---------------------------------------------------------------
VESSCOL_s& VESSCOL_s::operator = (const VESSCOL_s& a)
   {
...
//-----------------------------------------------------
void VESSCOL_s::SetFixFactors(float a, float b, float c)
   {
...
void VESSCOL_s::RemoveRoot(int i)
   {
   if (i < 0 || i >= vessnum) return;
   int par;
...
//----------------------------------------
int VESSCOL_s::GetRoots(LIST<int>* lst)
   {
   *lst = rootlist;
   int val, count = 0;
   lst->Reset();
...
int VESSCOL_s::GetOrphans(LIST<int>* lst)
{
   lst->Releasemem();
   int count = 0;
...
int VESSCOL_s::GetBlocked(LIST<int>* lst)
   {
   lst->Releasemem();
   int count = 0;
   for (int i = 0; i < vessnum; i++)
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
int VESSCOL_s::GetValidChildren(int vessid, LIST<int>* lst)
   {
...
//------------------------------
   int VESSCOL_s::GetAllChildren (int vessid, LIST<int>* lst)
   {
   if (vessid < 0 || vessid >= vessnum) return 0;
...
//-----------------------------------
   float* VESSCOL_s::GetFloatAttachPt(int i)
   {
...
//------------------------------------
int VESSCOL_s::OrphanizeSubtree(int a)
   {
   if (a < 0 || a >= vessnum)
      return 0;
...
//-----------------------------------
      ...
      //at the tail.
int VESSCOL_s::ConnectChildToParent(int child, int childpos, int par, int parpos)
   {
   LIST<int> lst;
   int val;
   int returnval = 1;
   ...
   while (lst.Get(&val))
      ...
      if (val == par)
         ...
         std::cerr<<"Creates an invalid loop!\n Give par another parent first\n.";
            strcpy_s(err,200, "Creates an invalid loop");
         return 0;
         }
      }
...
int VESSCOL_s::SplitVessel(int i, int pos)
   {
   if (i < 0 || i >= vessnum)
      {
      std::cerr<<"Invalid onpick in VESSCOL_s::SplitVessel()\n";
        strcpy_s(err,200, "Invalid onpick in SplitVessel");
      return 0;
      }

   if (pos <= 0 || pos >= vess[i].NPoints() - 1)
      {
      std::cerr<<"Invalid position in VESSCOL_s::SplitVessel()\n";
        strcpy_s(err,200, "Invalid position in SplitVessel()");
      return 0;
      }

   if ( (vessnum + 1) >= maxvess)
      {
      std::cerr<<"Insufficient extra vessels to make a new one in VESSCOL_s\n";
        strcpy_s(err,200, "Insufficient extra vessels");
      return 0;
      }
...
int VESSCOL_s::Reverse(int a)
   {
   if (a >= vessnum || a < 0) {std::cerr<<"Invalid id\n"; strcpy_s(err, 200, "Invalid id");return 0;}
   vess[a].Reverse();
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
int VESSCOL_s::MinMaxBox(float* min, float* max)
   {
   if (!vessnum) return 0;
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
float* VESSCOL_s::DoConnectedSample(int i, int nskip, int* num)
   {
...
//----------------------------------------------------
float* VESSCOL_s::DoConnectedAllowedSample(int i, int nskip, int* num)
   {
   float* fptr = GetFloatAttachPt(i);
...
int VESSCOL_s::Load(char* fname)
   ...
   if(!ifile)
      {
      std::cerr<<"VESSCOL_s::Load--Unable to open input file\n"; 
      strcpy_s(err,200, "Unable to open input file");
      return 0;
      }
      

   if(!LoadHeader(&ifile)) 
      {ifile.close(); std::cerr<<"Could not load header"; strcpy_s(err,200, "could not load header");return 0;}
   ...
   for (i = 0; i < vessnum; i++)
      ...
      if(!vess[i].Load(&ifile))
         {
         char s[100];
         sprintf_s(s, 100, "LOAD FAILED on vessel %i of %i", i + 1, vessnum);
         std::cerr<<s;
         strcpy_s(err,200, s);
         returnval = 0;
         break;
...
//---------------------------------------------
int VESSCOL_s::LoadWorldTree(int n, VESS_s* v, int* vox, float* nsteps)
   {
   Releasemem();
...
//------------------------------------
int VESSCOL_s::LoadVoxTree(int n, VESS_s* v, int* vox, float* nsteps)
   {
   Releasemem();
...
//---------------------------------------------
void VESSCOL_s::SetHeaderParams(int* _dims, float* _steps)
   {
   for (int i = 0; i < 3; i++)
      {
      dims[i] = _dims[i];
...
//------------------------------------------------------
int VESSCOL_s::AddWorldVess(VESS_s* avess)
   {
   int par;

   if (vessnum >= maxvess - 1)
      {
      strcpy_s(err,200, "Vessel maximum exceeded. Cannot add");
...
//-------------------------------------------------------
int VESSCOL_s::AddVoxVess(VESS_s* avess)
   {
...
//-----------------------------------------
int VESSCOL_s::Save(char* fname)
   {
   if (!fname)
      {
      std::cerr<<"bad file name\n";
        strcpy_s(err,200, "bad file name");
      return 0;
      }
   ...
   if (!count)
      ...
      std::cerr<<"No vessels picked for save\n";
        strcpy_s(err,200, "No vessels picked for save");
      return 0;
      }
   ...
   try
      {
      savevess = new VESS_s[count];
      if(!savevess)
         {
         std::cerr<<"Out of mem; cannot save\n";
         strcpy_s(err,200, "Out of mem; cannot save");
         return 0;
         }
      }
   catch(...)
  ...
  for (i = 0; i < vessnum; i++)
   ...
   std::ofstream ofile;
    ...
    if (!ofile)
       ...
       strcpy_s(err,200, "could not open output file");
       delete [] savevess; 
       return 0;
       }
...
int VESSCOL_s::SaveAll(char* fname)
   {
       std::ofstream ofile;
    ofile.open(fname);
    if (!ofile)
       {std::cerr<<"Could not open output file\n"; strcpy_s(err,200, "Could not open output file");return 0;}
    VESS_s tempvess;
...
//--------------------------------------------------------
int VESSCOL_s::SaveUnblocked(char* fname)
   {
   if (!fname)
      {
      std::cerr<<"bad file name\n";
...
//---------------------------------------------------------
int VESSCOL_s::SaveBranchpointFile(char* fname)
   {
   if (!fname)
      {
      std::cerr<<"bad file name\n";
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
   //PROTECTED::LOAD
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
int VESSCOL_s::LoadHeader(std::ifstream* ifile)
   {
   int returnval = FILEHEADERIO::LoadHeader(ifile);
    ...
    if (ndims != 3)
       ...
       strcpy_s(err,200, "File must have 3 dimensions");
       returnval = 0;
       }
    ...
    if(returnval)
       {
         ...
         try
            ...
            if(!vess)
               ...
               strcpy_s(err,200, "No mem for vessels");
               vessnum = 0;
               return 0;
               }
            }
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
         ...
         //parent opacity is 0
int VESSCOL_s::RecursiveChildAllowed(int a, LIST<int>* lst)
   {
   int count = 0;
   int childvals[2];
   int prox, dist;
...
int VESSCOL_s::RecursiveChildAll(int a, LIST<int>* lst)
   {
   int count = 0;
   int childvals[2];
...
//--------------------------
void VESSCOL_s::RenumberParent(VESS_s* v, LIST<int>* discards)
   {
...
//-----------------------------
void VESSCOL_s::RenumberChildren(VESS_s* v, LIST<int>* discards)
   {
   int childinfo[2];
...
//-----------------------------
int VESSCOL_s::Recount(int startval, LIST<int>* excludes)
   {
...
//---------------------------------------------
int VESSCOL_s::SplitChildArterial(int child, int childpos, int par, int parpos)
  {
   VESS_s headend, tailend;
  int counter;
  int i;
  ...
  if (vessnum >= (maxvess - 1))
    {
      std::cerr<<"You must increase maxvess in VESSCOL_s.h--not enough vessels\n";
        strcpy_s(err,200, "You must increase maxvess in VESSCOL_s.h");
      return 0;
      }
...
//---------------------------------------------
int VESSCOL_s::SplitChildVenous(int child, int childpos, int par, int parpos)
  {
   VESS_s headend, tailend;
  int counter;
  int i;
