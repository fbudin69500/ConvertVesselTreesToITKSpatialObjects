...
//------------------------------
MESHCOL::MESHCOL(const MESHCOL& a)
   {
   Initmem();
   nmesh = a.nmesh;
   nstrips = a.nstrips;
   ...
   if(nmesh)
      ...
      mesh = new MESH[nmesh];
      if (!mesh)
         std::cerr<<"Out of mem in MESHCOL()\n";
      else
         {
...
//--------------------------------------------
MESHCOL& MESHCOL::operator = (const MESHCOL& a)
   {
...
//---------------------------------
void MESHCOL::Initmem()
   {
   nmesh = 0;
   mesh = 0;
   }
//-----------------------------------
int MESHCOL::AllocMemForNMeshes(int n)
   {
   Releasemem();
   if (n >= 0)
      {
...
//-----------------------------------
int MESHCOL::CreateSurfaces(SURFACECOL* mc, unsigned char* maskfile, float* s)
   {
   Releasemem();
...
int MESHCOL::CreateVessels(VESSCOL* vc, int extraslots)
   {
   Releasemem();
   ...
   for (int i = 0; i < vc->GetVessnum(); i++)
     ...
     if (num <= 0) {std::cerr<<"\n\nMESHCOL::CreateVessels():Failure to create vessel mesh for vess "<<i<<" with "<<num <<" points; continuing\n\n";continue;}
     

      if(!mesh[i].CreateVessel(fptr, num, vc->GetDims(), vc->GetSteps()))
         {
         Releasemem();
         std::cerr<<"\n\nMESHCOL::CreateVessels():Failure to create vessel mesh for vess "<<i<<" with "<<num <<" points\n\n";
         return 0;
         }
     }
   return 1;
 ...
 //----------------------------------------------------------
int MESHCOL::CreateVessel(VESSCOL* vc, int a)
   {
   if (nmesh <= a) return 0;
   if(!mesh) return 0;
...
int MESHCOL::GetMinMax(float* mins, float* max)
   {
   if (!nmesh) return 0;
