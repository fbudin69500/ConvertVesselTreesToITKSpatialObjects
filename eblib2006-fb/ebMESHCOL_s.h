#ifndef _ebMESHCOL_s_h
#define _ebMESHCOL_s_h
...
class MESHCOL_s
   ...
   public:
      MESHCOL_s();
      ~MESHCOL_s();
      MESHCOL_s(const MESHCOL_s& a);
      MESHCOL_s& operator = (const MESHCOL_s& a);
      ...
      MESH_s* GetMesh(int aa)    {return &mesh[aa];}
      ...
      int CreateVessel(int aa, VESS_s* vess, int* d, float* s) 
      ...
      int CreateVessels(VESSCOL_s* vc, int extraslots = 0);
      int CreateVessel(VESSCOL_s* vc, int a);      //put something into an index in existence
