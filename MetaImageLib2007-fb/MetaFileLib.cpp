...
bool MF_SkipToVal(FILE *fp)
   ...
   while((c == sepChar || isspace(c)) && !feof(fp))
      fscanf(fp, "%c", &c);
   ungetc(c, fp);
   if(feof(fp))
      {
...
bool MF_Read(FILE *fp, int nFields, MF_FieldRec *field, int fTerm, bool fromTopOfFile, char _sepChar)
   ...
   while(1)
      ...
      i = 0;
      s[i] = c;
      while(i<255 && isprint(s[i]) && s[i]!=sepChar && !feof(fp))
         s[++i] = getc(fp);
      if(i>=255)
         return MF_IsComplete(nFields, field);
      if(feof(fp))
      ...
      ungetc(s[i], fp);
      s[i+1] = '\0';
      while((s[i] == sepChar || isspace(s[i]) || !isprint(s[i])) && i>0)
         s[i--] = '\0';
      ...
      if(!found)
         {
         std::cout << "Unknown field = " << s << std::endl;
         if(!fgets(s, 255, fp))
            std::cout << "   error getting value." << std::endl;
         std::cout << "Unknown value = " << s << std::endl;
         }
      }
